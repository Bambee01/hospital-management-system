...
select employee_id,lower (position),ssn,lower (name) from physician
...
CREATE TABLE Affiliated_With (
          Physician INT NOT NULL
            CONSTRAINT fk_Physician_Employee_ID REFERENCES Physician(Employee_ID),
          Department INT NOT NULL
            CONSTRAINT fk_Department_DepartmentID REFERENCES Department(DepartmentID),
          PrimaryAffiliation VARCHAR NOT NULL,
            PRIMARY KEY(Physician, Department)
);
select * from Affiliated_With

CREATE FUNCTION DBO.GetPhysicianByDepartment (@department_id int)
returns table 
as
return(
          select * from physician where employee_id in  (select physician from Affiliated_With
      where Department = @department_id)
      
      )
  select * from GetPhysicianByDepartment (1)
...
CREATE TABLE Patient (
  ...
  PCP INT NOT NULL
    CONSTRAINT fk_Physician_EmployeeID REFERENCES Physician(Employee_ID)
);
...
CREATE TABLE Appointment (
  ...
  Physician INTEGER NOT NULL
    CONSTRAINT fk_Physician_Employees REFERENCES Physician(Employee_ID),
...
CREATE TABLE Prescribes (
       Physician INT NOT NULL
       CONSTRAINT fk_Physician_EmployeessID REFERENCES Physician(Employee_ID),
...
CREATE TABLE Room (
        ...
        Unavailable varchar(100) NOT NULL,
...
CREATE TABLE On_Call (
        Nurse INT NOT NULL
        CONSTRAINT fk_Nurse_EmployeesID REFERENCES Nurse(EmployeeID),    
        BlockFloor INT NOT NULL,
        BlockCode INT NOT NULL,
...
CREATE TABLE Stay (
        ...
        Patient INT NOT NULL
        CONSTRAINT fk_PatientsSSN REFERENCES Patient(SSN),
        Room INT NOT NULL
...
CREATE TABLE Undergoes (
          Patient INT NOT NULL
          CONSTRAINT fk_PatientssSSN REFERENCES Patient(SSN),
          Procedures INT NOT NULL
          CONSTRAINT fk_ProceduresCode REFERENCES Procedures (Code),
          Stay INTEGER NOT NULL
          CONSTRAINT fk_Stay_StayID REFERENCES Stay(StayID),
          Date DATETIME NOT NULL,
          Physician INT NOT NULL
          CONSTRAINT fk_PhysicianssEmployeeID REFERENCES Physician(Employee_ID),
          AssistingNurse INT
          CONSTRAINT fk_NursesEmployeeID REFERENCES Nurse(EmployeeID),
...
INSERT INTO physician VALUES(10,'PAUL REGGIE','SENIOR ATTENDING PHYSICIAN',1001111060)
